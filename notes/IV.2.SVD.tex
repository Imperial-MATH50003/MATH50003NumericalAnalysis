...
We will use induced matrix norms, in particular the 2-norm of a matrix $A \ensuremath{\in} \ensuremath{\bbC}^{m \ensuremath{\times} n}$ is defined via
\[
...
\]
The matrix 1- and $\ensuremath{\infty}$-norms have simple definitions in terms of column/row sums (see appendix). On the other hand, the 2-norm has no simple formula. 
...
To show the SVD exists we first establish some properties of a \emph{Gram matrix} ($A^\ensuremath{\star} A$), which has within its eigendecomposition part of the SVD. The Gram matrix is best viewed as the matrix of inner products of the columns of a matrix: if $A = \begin{bmatrix} \ensuremath{\bm{\a}}_1 | \ensuremath{\cdots} | \ensuremath{\bm{\a}}_m \end{bmatrix}$ then the Gram matrix is the Hermitian matrix
\[
A^\ensuremath{\star} A = \begin{bmatrix} \ensuremath{\bm{\a}}_1^\ensuremath{\star} \ensuremath{\bm{\a}}_1 & \ensuremath{\cdots} &\ensuremath{\bm{\a}}_1^\ensuremath{\star} \ensuremath{\bm{\a}}_m \\
                        \ensuremath{\vdots} &  \ensuremath{\ddots} & \ensuremath{\vdots} \\
                        \ensuremath{\bm{\a}}_m^\ensuremath{\star} \ensuremath{\bm{\a}}_1 & \ensuremath{\cdots} &\ensuremath{\bm{\a}}_m^\ensuremath{\star} \ensuremath{\bm{\a}}_m \end{bmatrix} \ensuremath{\in} \ensuremath{\bbC}^{m \ensuremath{\times} m}.
\]
...
\]
The fact that real $A$  implies $Q \ensuremath{\in} O(n)$ (i.e. has real entries) follows from direct calculation since we can choose the first entry of the eigenvector to be real.
...
\]
Now define $U := AV \ensuremath{\Sigma}^{-1}$. Since $A^\ensuremath{\star} A V = V \ensuremath{\Sigma}^2$ we can verify that $U$ is unitary:
\[
...
\begin{proposition}[diagonal/orthogonal 2-norms] If $\ensuremath{\Lambda}$ is diagonal with entries $\ensuremath{\lambda}_k$ then $\|\ensuremath{\Lambda}\| = \max_k |\ensuremath{\lambda}_k|$. If $Q$ is orthogonal then $\|Q\| = 1$.
...
\]
is the best 2-norm approximation of $A$ by a rank $k$ matrix, that is, for all rank-$k$ matrices $B$, we have  $\|A - A_k\| \ensuremath{\leq} \|A -B \|.$
...
\[
\|A \ensuremath{\bm{\u}}\|^2 = \|U \ensuremath{\Sigma}_k \ensuremath{\bm{\c}}\|^2 = \|\ensuremath{\Sigma}_k \ensuremath{\bm{\c}}\|^2 =
\sum_{j=1}^{k+1} (\ensuremath{\sigma}_j c_j)^2 \ensuremath{\geq} \ensuremath{\sigma}_{k+1}^2 \| \ensuremath{\bm{\c}}  \|^2,
\]
i.e., $\|A \ensuremath{\bm{\u}}\| \ensuremath{\geq} \ensuremath{\sigma}_{k+1} \|\ensuremath{\bm{\u}}\|$, where we use the fact that
\[
