...
\br[f(\ensuremath{\theta}) {\E^{-\I k \ensuremath{\theta}} \over -\I k}]_0^{2\ensuremath{\pi}} + {1 \over \I k} \ensuremath{\int}_0^{2\ensuremath{\pi}} f'(\ensuremath{\theta}) \E^{-\I k \ensuremath{\theta}} {\rm d}\ensuremath{\theta} \\
&=  \br[f'(\ensuremath{\theta}) {\E^{-\I k \ensuremath{\theta}} \over (-\I k)^2 }]_0^{2\ensuremath{\pi}} - {1 \over k^2} \ensuremath{\int}_0^{2\ensuremath{\pi}} f''(\ensuremath{\theta}) \E^{-\I k \ensuremath{\theta}} {\rm d}\ensuremath{\theta} \\
...
For periodic functions the Trapezium rule simplies and has a lot of nice properties (some of which we saw in the first lab). For concreteness we define a specific normalisation for the Trapezium rule as follows:
...
To perform computations with Fourier expansions we need to compute the Fourier coefficients, and the periodic Trapezium rule provides a natural way to do so. We will now use the Trapezium rule to approximate Fourier coefficients and expansions:
...
\begin{theorem}[discrete Fourier coefficients] If $\vchatf \ensuremath{\in} \ensuremath{\ell}^1$ then
\[
...
Note that there is redundancy, in that higher order discrete Fourier coefficients are exactly the same as certain low order ones. This phenomena is called \emph{aliasing}:
...
\end{pmatrix}.
\]
We can  prove \emph{convergence} of an approximate Fourier series using the discrete Fourier coefficients  whenever $f$ has absolutely summable coefficients. We will prove the result here in the special case where the negative coefficients are zero. That is, $\hat f_0^n, \ensuremath{\ldots}, \hat f_{n-1}^n$ are approximations of the Fourier\ensuremath{\endash}Taylor coefficients.
...
In the problem sheet we investigate some examples of Fourier and discrete Fourier coefficients and explore the relationship between smoothness, decay in Fourier coefficients, and convergence of approximate Fourier expansions. We also extend the convergence and interpolation results to the non-Taylor case, that is, where the negative Fourier coefficients may be zero. 

In the lab we explore numerical computation of Fourier coefficients and convergence of Fourier expansions. We also observe the \emph{Gibb's phenomena}, where approximate Fourier expansions overshoot near discontinuities.
