...
\textbf{Numerical Analysis MATH50003 (2025\ensuremath{\endash}26) Problem Sheet 8}

In this PS we explore the relationship between Fourier series and discrete Fourier coefficients.  We extend some of the constructions from the Taylor case (where all negative Fourier coefficients are zero) to the more general case. We also study the relationship between regularity of functions and decay of their Fourier coefficients. Finally, we see how the Discrete Fourier Transform (DFT) Can be adapted to create the Discrete Cosine Transform (DCT).

We begin with a simple example of computing Fourier and discrete Fourier coefficients. Note in these examples we do not need to actually compute integrals:
...
\rule{\textwidth}{1pt}
A basic feature of analysis is the relationship between \emph{smoothness} (or \emph{regularity}) of a function and decay of its Fourier coefficients. This becomes important in numerical analysis for deducing the speed that an approximate Fourier expansion  converges to a function. In the following problem we see directly that more differentiability implies faster decay of coefficients and convergence of finite Fourier expansions.
...
\rule{\textwidth}{1pt}
In the notes we focussed on the Taylor series case for simplicity. Here we explore extending this to the more general case.  In particular, we can (1) exactly recover trigonometric polynomials, (2) prove convergence rates for approximate Fourier expansions and (3) prove an interpolation property.
...
\rule{\textwidth}{1pt}
We now turn our attention to cosine analogues of Fourier expansions. These are not only important for approximating  even functions but also non-periodic functions by the change-of-variables $f(\cos \ensuremath{\theta})$ which turn a smooth function to a smooth, periodic even function (and is linked to Chebyshev expansions arising in orthogonal polynomials, which is discussed later). We see here that the properties of the Discrete Fourier Transform can be extended to this specific case in the construction of the Discrete Cosine Transform:
