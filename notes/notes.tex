...
\begin{enumerate}
    ...
    \item III.2 LU and Cholesky Factorisations: we look at computing a factorisation of a square matrix as a product of a lower and upper triangular matrix, including the special case where the matrix is symmetric positive
...
Here we are constructing underlying computational tools that are important in applications, which we discuss later. 
...
%\item III.3 Polynomial Regression: often in data science one needs to approximate data by a polynomial.
...
% \chapter{Orthogonal ploynomials}
