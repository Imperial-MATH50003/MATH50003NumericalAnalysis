...
We investigate how interval arithmetic can be used for computing rigorous bounds of calculations. This includes both simple arithmetic expressions as well as more complicated mathematical expressions like computing functions from their Taylor series.  In particular,  we show that one can compute fairly sharp bounds of $\sin 1$. The lab takes this further and  implements these computations on a computer, allowing for much higher accuracy rigrouous computations.
...
We complete the proofs of interval multiplication and division that were omitted in the notes, and ask for the generalisation to cases not considered in the notes:
...
\rule{\textwidth}{1pt}
The next problem computes rigorous bounds on $\sin 1$ using interval arithmetic. We begin with a very simple arithmetic computation with floating point intervals, corresponding to the first two terms of the Taylor series of $\sin x$:
