...
We discuss polynomial interpolation and see how it can be used to compute integrals with higher accuracy than rectangular or trapezium rules. We also discuss regression, where more data is used than the degree of the polynomial,
leading to a robust approach  produced by solving a rectangular least squares problem.

\item IV.2 Singular Value Decomposition and Compression: We discuss the singular value decomposition, which gives the 2-norm of a matrix.
We also discuss how it leads to an effective approach to compression, in particular, it allows for the computation of the best low-rank approximation to a matrix, in the sense that
the matrix 2-norm is minimised. 
...
% \input{IV.3.DifferentialEquations}
  ...
  We have seen intuitive numerical methods for computing derivatives and integrals, 
  primarily based on representing functions by their values at a grid of points.
  ...
  which will later be adapted in the form of orthogonal polynomials for non-periodic functions that are smooth within an interval.
  

  \begin{enumerate}
      \item V.1 Fourier Expansions: we discuss Fourier series and their usage in approximating periodic functions, using the Trapezium rule to compute the Fourier coefficients.
      \item V.2 Discrete Fourier Transform: The Trapezium rule approximation can be recast as a unitary matrix, known as the Discrete Fourier Transform (DFT). This is used to prove interpolation properties.
            \item V.3 Convolutions: Convolutions are natural ways of representing operators that commute with translations, and underly Convolutional Neural Networks which have advantages for problems with natural
            translation symmetry (for example, number recognisition which should be invariant to where the number is in an image). 
            We discuss how discrete convolutions can be diagonalised using the DFT leading to fast convolution when implemented via the Fast Fourier Transform (FFT).
 \end{enumerate}

 \input{V.1.Fourier}
 \input{V.2.DFT}
  \input{V.3.Convolutions}
...
We can approximate the coefficients $c_k \approx c_k^n$ using quadrature, just as we did with Fourier series.

Why not use monomials \ala\  Taylor series? Hidden in the discussion on Fourier series was that we could effectively
compute Taylor coefficients by evaluating on the unit circle in the complex plane, only if the radius of convergence
was 1. Many functions are smooth on say $[-1,1]$ but have non-convergent Taylor series, e.g.  {\it Runge's function}:
$$
{1 \over 25x^2 + 1},
$$
the function we have seen in Lab 7 is difficult to interpolate. While orthogonal polynomials span the same space as monomials, orthogonal polynomials are much more stable and can effectively and accurately
...
Due to time constraints we will only discuss one, extremely important, family of orthogonal polynomials:



\begin{enumerate}
    \item VI.1 Chebyshev polynomials: We begin with Chebyshev polynomials, a canonical example of orthogonal polynomials that are in fact Fourier series in disguise. These demonstrate many of the essential properties of general orthogonal polynomials.
    % \item VI.2 General Orthogonal Polynomials: For non-periodic functions we consider the definition of orthogonal polynomials, and discuss their basic properties.
%     \item VI.3 Classical Orthogonal Polynomials: For certain weights, orthogonal polynomials are classical and have addition structure that are useful for computations.
%     \item VI.4 Gaussian Quadrature: Finally, we revisit the  problem of computing integrals, and see that using orthogonal polynomials we can derive much more accurate methods.
    \end{enumerate}
...
Fourier and orthogonal polynomial expansions also lead to very effective schemes for solving differential equations
and many other applications. In addition to numerics, OPs play an important role in many mathematical areas
including functional analysis, integrable systems, singular integral equations,
complex analysis, and random matrix theory.


\input{VI.1.ChebyshevPolynomials}
...
In these notes we explored some basic numerical algorithms for integration, differentiation, root-finding, and
...
\item Finite Element Methods (FEM): finite elements are powerful techniques for solving partial differential
equations using specially designed bases built from piecewise polynomials. Orthogonal polynomials underly the quadrature rules  used to turn PDEs into
sparse linear system. \href{https://www.firedrakeproject.org}{Firedrake} is a popular software package for
...
\chapter{ConditionNumbers}
\input{A.ConditionNumbers}
