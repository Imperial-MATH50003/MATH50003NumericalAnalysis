...
A = L L^\ensuremath{\top},
\]
i.e., $U = L^\ensuremath{\top}$, but now $L$ does not necessarily have 1s on the diagonal. This provides an algorithmic way to \emph{prove} that a matrix is symmetric positive definite, and is roughly twice as fast as the LU factorisation to compute.
...
In the problem sheet we see how the Cholesky factorisation can be used to prove positive definiteness of both specific examples of small matrices, but also special families of arbitrarily large matrices. We also see the \emph{reverse} Cholesky factorisation of the form $A = U U^\top$. In the  lab we investigate the implementation of the Cholesky factorisation, and see how bandedness can be incorporated into the implementation to achieve better complexity.
