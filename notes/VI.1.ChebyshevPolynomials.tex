...
This connection between the Vandermonde-like matrix $V$ and the DCT guarantees that we can apply and invert $V$ in $O(n \log n)$ time  which is much faster than the $O(n^3)$ time required for a general matrix. Moreover, because the DCT is orthogonal it can be applied and inverted \emph{stably}: we do not see  the instability observed with monomials in Lab 7. 

The power and reliability of Chebyshev interpolation is the starting point of what makes them such powerful tools in computation.
