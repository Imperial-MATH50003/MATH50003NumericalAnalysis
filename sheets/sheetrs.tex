...
\[
\int_{-1}^1 U_n(x) p(x) \sqrt{1-x^2} {\rm d}x = 0.
\]
\textbf{SOLUTION}

PS9 Q4(a) implies that $\{U_k(x)\}$ are a basis of polynomials. Hence we can  expand
\[
p(x) = \ensuremath{\sum}_{k=0}^m c_k U_k(x)
\]
for some coefficients $c_k$. Then:
\[
\int_{-1}^1 U_n(x) p(x) \sqrt{1-x^2} {\rm d}x 
= \ensuremath{\sum}_{k=0}^m c_k \int_{-1}^1 U_n(x) U_k(x) \sqrt{1-x^2} {\rm d}x  = 0
\]
since Q9 4(b) shows that
\[
\int_{-1}^1 U_n(x) U_k(x) \sqrt{1-x^2} {\rm d}x  = 0
\]
when $k \ensuremath{\neq} n$ (which is always true since $k < n$).
