...
One of the most fundamental problems in linear algebra is solving linear systems, for a field $\ensuremath{\bbF}$ (for us either $\ensuremath{\bbR}$ or $\ensuremath{\bbC}$), invertible matrix $A\ensuremath{\in} \ensuremath{\bbF}^{n \ensuremath{\times} n}$ and vector $\ensuremath{\bm{\b}} \ensuremath{\in} \ensuremath{\bbF}^n$, find a $\ensuremath{\bm{\x}} \ensuremath{\in} \ensuremath{\bbF}^n$ such that
\[
A \ensuremath{\bm{\x}} = \ensuremath{\bm{\b}}.
\]
This can of course be done via Gaussian elimination, uing row swaps (or \emph{pivotting}) if a zero is encountered on the diagonal, which can readily be implemented on a computer. However, a basic observation makes the practical implementation more straightforward and easier to apply to multiple right-hand sides. 

In particular, Gaussian elimination is equivalent to an \emph{LU factorisation}:
\[
A =L U
\]
where $L$ is lower triangular and $U$ is upper triangular. Thus if compute $L$ and $U$ we can deduce
\[
\ensuremath{\bm{\x}} = A^{-1} \ensuremath{\bm{\b}}= U^{-1} L^{-1} \ensuremath{\bm{\b}}
\]
where $L$ and $U$ are invertible via forward- and back-substitution.

On the other hand, Gaussian elemination with pivotting (row-swapping) is equivalent to a \emph{PLU factorisation}:
\[
...
Finally, in the special case where $A$ is  a real square \emph{symmetric positive definite} ($A \ensuremath{\in} \ensuremath{\bbR}^{n \ensuremath{\times} n}$ such that $A^\ensuremath{\top} = A$ and $\ensuremath{\bm{\x}}^\ensuremath{\top} A \ensuremath{\bm{\x}} > 0$ for all $\ensuremath{\bm{\x}} \ensuremath{\in} \ensuremath{\bbR}^n$, $\ensuremath{\bm{\x}} \ensuremath{\neq} 0$) matrix the LU factorisation has a special form called the \emph{Cholesky factorisation}:
\[
A = L L^\ensuremath{\top},
\]
